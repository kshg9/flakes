...
- [x] **sops** — done pivot 2026-08-09, POC verified on sandbox (2nd host): **TWO-KEY, per-host-file model** — personal editing key `~/.config/sops/age/keys.txt` (sops CLI key) + per-host boot key `/var/lib/sops-nix/key.txt` (`generateKey`). `nixos/features/sops.nix` is GENERIC (keyFile + CLI only); `sops.secrets.*` + `defaultSopsFile` are declared PER-HOST in each host's `configuration.nix` (uriel: `secrets/uriel.yaml` `[&kdj,&uriel]`; sandbox: `secrets/sandbox.yaml` `[&kdj,&sandbox]`, boot key swapped in via virtiofs `~/Downloads`→`/mnt/host`, biyoo given `keys` group). Boot-key persistence lives in `features/impermanence.nix`. `.sops.yaml` has real per-file `creation_rules` + real pubkeys all three. Non-interactive add/edit = `jaq -Rs .`+`sops set --value-file FILE INDEX VALUE`. Full add-a-host runbook + jaq traps in `KB/sops.md`. **kdj's ssh keys are passphrase-protected → never usable as boot keys (verified)**.
